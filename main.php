...
function findMatches($conn, $current_user_id, $city, $state, $ignore_location, $full_match)
{
    ...
    $match_condition = $full_match ? "HAVING COUNT(needs_match) > 0 AND COUNT(offers_match) > 0 AND COUNT(needs_match) = COUNT(offers_match)" : "HAVING (COUNT(needs_match) > 0 AND COUNT(offers_match) = 0) OR (COUNT(needs_match) = 0 AND COUNT(offers_match) > 0)";
